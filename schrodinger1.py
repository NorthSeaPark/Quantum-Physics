...
"""
import sys
sys.path.remove('/Library/Frameworks/Python.framework/Versions/3.7/lib/python3.7/site-packages')
sys.path.append('/Users/franco_lu/anaconda3/lib/python3.7/site-packages')
import numpy as np
...
class Schrodinger(object):
    ...
    def __init__(self, x, psi_x0, V_x,
        ...
        self.x, psi_x0, self.V_x = map(np.asarray,
                                       (x, psi_x0, V_x))  # using the map function convert the input to an array
    ...
    # Allows to set the value of psix given a function
    def _set_psi_x(self, psi_x):
        self.psi_mod_x = (psi_x * np.exp(-1j * self.k[0] * self.x)
                          * self.dx / np.sqrt(2 * np.pi))

    # Returns the value of psix given a function
    def _get_psi_x(self):
        return (self.psi_mod_x * np.exp(1j * self.k[0] * self.x)
                * np.sqrt(2 * np.pi) / self.dx)

    # Allows to set the calue of psik given a function
    def _set_psi_k(self, psi_k):
        self.psi_mod_k = psi_k * np.exp(1j * self.x[0]
                                        * self.dk * np.arange(self.N))

    # Returns the vaule of psik of the input function
    def _get_psi_k(self):
    ...
    # set new values given the change in t
    def _set_dt(self, dt):
        if dt != self.dt_:
            self.dt_ = dt
    ...
    psi_x = property(_get_psi_x, _set_psi_x)  # rest psix ans j
